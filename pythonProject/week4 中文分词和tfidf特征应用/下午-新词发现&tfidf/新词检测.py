...
class NewWordDetect:
    ...
    #按照窗口长度取词，并记录左邻右邻。可以固定word_length=5看结果，记录长度为4个字符的字符串，放在word_count计数
    ...
    def calc_pmi(self):
        ...
        for word, count in self.word_count.items():
            ...
            self.pmi[word] = math.log(p_word / p_chars, 10) / len(word) # 取对数，底数是10
            # print(math.log(p_word / p_chars, 10),'math.log(p_word / p_chars, 10)')
            # 6.454058928447586
        return
...
if __name__ == "__main__":
    ...
    # 计算tfidf最高的几个词，reverse=True是从大到小降序
