...
def tf_idf_topk(tfidf_dict, paths=[], top=10,print_word=True):
    ...
    for text_index,text_tfidf_dict in tfidf_dict.items():
        word_list = sorted(text_tfidf_dict.items(), key=lambda x:x[1], reverse=True) #根据第二个值从小到大排序
...
def main():
    ...
    # print(paths, 'paths')
    # ['sports.txt', 'auto.txt', 'world.txt', 'science.txt', 'health.txt', 'finance.txt'] paths
