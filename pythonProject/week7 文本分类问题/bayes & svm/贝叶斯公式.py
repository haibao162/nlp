...
class BayesApproach:
    ...
    def load(self, path):
        ...
        with open(path, encoding="utf8") as f:
            for line in f:
                ...
                word_freq = self.class_name_to_word_freq[class_name] # 一个类别下的一个词的出现次数
                #记录每个类别下的词频
                for word in words:
    ...
    def freq_to_prob(self):
        #样本概率计算
        total_sample_count = sum(self.p_class.values()) # 多少类别
        ...
        for class_name, word_freq in self.class_name_to_word_freq.items():
            total_word_count = sum(count for count in word_freq.values()) # 一个类别下所有词出现的总次数
            for word in word_freq:
                #加1平滑，避免出现概率为0，计算P(wn|x1)。 x1类别下出现词wn的概率。
