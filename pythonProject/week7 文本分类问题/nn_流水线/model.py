...
class CNN(nn.Module):
    ...
    def forward(self, x): #x : (batch_size, max_len, embeding_size)
        # max_length是外部传入的不确定的，hidden_size是模型的，所以将max_len * embeding_size转置成
        # embeding_size * max_len 和 hidden_size * kernel_size去卷积，embeding_size是等于hidden_size的，代表向量维度。
...
class BertMidLayer(nn.Module):
    def __init__(self, config):
        super(BertMidLayer, self).__init()
        self.bert = BertModel.from_pretrained(config["pretrain_model_path"], return_dict=False)
        self.bert.config.output_hidden_states = True
    
    def forward(self, x):
        # 获取transform层每一层计算的结果
