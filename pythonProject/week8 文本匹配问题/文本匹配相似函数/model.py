...
class SentenceMatchNetwork(nn.Module):
    def __init__(self, config):
        ...
        self.encoder = nn.Sequential(nn.LSTM(hidden_size, hidden_size, bidirectional=True,
                                          batch_first=True), GetFirst(),
                                          nn.ReLU(),
...
if __name__ == "__main__":
    ...
    # y = model(s1, s2, l)
    # dataloader会把我们的数据按照batch_size组装起来，变成二维数组。
    y = model(torch.LongTensor([[1,2,3,0,2,2,0,0]]), torch.LongTensor([[1]]))
