...
# Wi * R(qi, d) , Wi是IDF，可N / n(qi)正比。如果n(qi)比较小，IDF就比较大，这样在在其它文章中出现的次数就比较少。
#  计算查询 q 和文档 d 的相关性分数
...
class BM25:
    ...
    def _initialize(self, corpus: Dict):
        ...
        for word in self.docContainedWord.keys():
            ...
            idf = math.log(self.corpus_size - doc_nums_contained_word + 0.5) - math.log(
                doc_nums_contained_word + 0.5)
            self.idf[word] = idf
            idf_sum += idf
            if idf < 0:
                negative_idfs.append(word)
        
        # 计算negative_idfs里的idf
