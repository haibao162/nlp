            ...
            for line in f:
                ...
                if isinstance(line, dict):
                    ...
                    for question in questions:
                        input_id = self.encode_sentence(question) # 获取句子的onehot
                        input_id = torch.LongTensor(input_id)
                        self.knwb[self.schema[label]].append(input_id) # "类别的id": [input_id, input_id]
                #加载测试集
                else:
                    self.data_type = 'test'
...
def load_data(data_path, config, shuffle=True):
    ...
    # print(len(dg), 'leneee') # 200
    dl = DataLoader(dg, batch_size=config["batch_size"], shuffle=shuffle)
    # print(list(dl.dataset.knwb.items())[0][1], 'dataset.knwb')
    return dl
...
if __name__ == "__main__":
    ...
    print(dg[1])
    print("---------------")
    valid_data = load_data(Config["train_data_path"], Config, shuffle=False)
