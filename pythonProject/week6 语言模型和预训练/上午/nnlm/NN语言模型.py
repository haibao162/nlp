...
class LanguageModel(nn.Module):
    def __init__(self, input_dim, vocab):
        ...
        self.embedding = nn.Embedding(len(vocab) + 1, input_dim, padding_idx=0) # input_dim 128层隐藏层
        # num_layers过两层rnn
    ...
    def forward(self, x, y = None):
        x = self.embedding(x) # batch_size * sen_len * input_dim
        x, _ = self.layer(x) # batch_size * sen_len * input_dim
...
def build_vocab(vocab_path):
    ...
    with open(vocab_path, encoding='utf8') as f:
        for index, line in enumerate(f):
            char = line[:-1] #去掉结尾换行符
            vocab[char] = index + 1 #留出0位给pad token, 词映射成数字
        vocab["\n"] = 1
    return vocab
...
def build_sample(vocab,window_size,corpus):
    ...
    window = corpus[start:end] # 输入
    target = corpus[end] # 输出
    # print(window, target)
...
def train(corpus_path, save_weight=True):
    ...
    print(corpus_path, 'corpuscorpus')
