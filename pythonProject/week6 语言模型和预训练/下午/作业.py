...
# print(state_dict["embeddings.word_embeddings.weight"].numpy().shape, 'word_embeddingsword_embeddings')
...
class DiyBert:
   ...
   #归一化层
    ...
    def get_weights_length(self):
        result = []
        # print(self.word_embeddings.shape, 'word_embeddings')
        self.word_embeddings = state_dict["embeddings.word_embeddings.weight"].numpy()
        self.position_embeddings = state_dict["embeddings.position_embeddings.weight"].numpy()
        self.token_type_embeddings = state_dict["embeddings.token_type_embeddings.weight"].numpy()
        self.embeddings_layer_norm_weight = state_dict["embeddings.LayerNorm.weight"].numpy()
        ...
        self.transformer_weights = []
        result.append({
            "word_embeddings": self.word_embeddings.shape
        })
        result.append({
        ...
        result.append({
            "embeddings_layer_norm_bias": self.embeddings_layer_norm_bias.shape
        })
        ...
        for i in range(self.num_layers):
            ...
            dic = [[q_w, "q_w"], [q_b, "q_b"], [k_w, "k_w"], [k_b, "k_b"], [v_w, "v_w"], [v_b, "v_b"],
                   [attention_output_weight, "attention_output_weight"], [attention_output_bias, "attention_output_bias"],
                   [attention_layer_norm_w, "attention_layer_norm_w"], [attention_layer_norm_b, "attention_layer_norm_b"],
                   [intermediate_weight, "intermediate_weight"], [intermediate_bias, "intermediate_bias"],
                   [output_weight, "output_weight"], [output_bias, "output_bias"], [ff_layer_norm_w, "ff_layer_norm_w"],
                   [ff_layer_norm_b, "ff_layer_norm_b"]]
            for item in dic:
                result.append({
                    item[1] + str(i+1) : item[0].shape
                })
        ...
        self.pooler_dense_bias = state_dict["pooler.dense.bias"].numpy()
        result.append({
            "pooler_dense_weight": self.pooler_dense_weight.shape
        })
        result.append({
            "pooler_dense_bias": self.pooler_dense_bias.shape
        })
        return result
...
print(torch_sequence_output, 'torch_sequence_output')
