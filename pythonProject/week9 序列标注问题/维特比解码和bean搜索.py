...
def beam_search(fence, beam_size):
    ...
    height = fence.height
    starter = Path()
    # print(starter.__len__(), 'starter')
    beam_buffer = [starter]
    # print(beam_buffer, 'beam_buffer')
    new_beam_buffer = []
    while True:
        for path in beam_buffer:
            ...
            for h in range(height):
                node = [h, path_length]
                # print(node, 'node')
                # [0, 0] node [1, 0] node [2, 0] node [3, 0] node
                # [0, 1] node [1, 1] node [2, 1] node [3, 1] node
                # [0, 2] node [1, 2] node [2, 2] node [3, 2] node
                # [0, 3] node [1, 3] node [2, 3] node [3, 3] node
                # [0, 4] node [1, 4] node [2, 4] node [3, 4] node
                # [0, 5] node [1, 5] node [2, 5] node [3, 5] node
